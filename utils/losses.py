...
def label_smoothing_criterion(alpha=0.1, distribution='uniform', std=0.5, reduction='mean'):
    def _label_smoothing_criterion(logits, labels):
        ...
        if distribution == 'uniform':
            ...
            soft_labels = torch.from_numpy(dist[labels.cpu().numpy()]).cuda()
        else:
            raise NotImplementedError
...
def compute_trivial_ot_loss(input, target, M):
...
class TrivialOT(nn.Module):
    ...
    def forward(self, logits, target):
        ...
        if self.lambd == 0:
            ...
            preds = self.normalization(logits)
            loss = compute_trivial_ot_loss(preds, target, self.M)
            if self.reduction == 'none':
                return base_l + self.lambd*loss
            elif self.reduction == 'mean':
                return base_l + self.lambd*loss.mean()
...
def trivial_ot_loss(n_classes, exp, normalization='softmax', reduction='mean'):
