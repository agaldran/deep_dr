...
parser.add_argument('--load_path', type=str, default='experiments/2020-02-14-17:42:48', help='path to saved model')
parser.add_argument('--pretrained', type=str2bool, nargs='?', const=True, default=True, help='from pretrained weights')
parser.add_argument('--tta', type=str2bool, nargs='?', const=True, default=False, help='use tta')
...
if __name__ == '__main__':
    '''
    Example:
    python test.py --tta True --load_path experiments/ce_reg_1e2 --csv_out results/ce_reg_1e2.csv
    '''
    data_path = 'data'
    ...
    if tta:
        probs, preds, labels = test_cls_tta(model, test_loader)
    else:
