...
if __name__ == "__main__":
    # handle paths
    path_raw_data = '/home/agaldran/Desktop/data/deepdr_data/Online-Challenge1&2-Evaluation'
    path_data_out = 'data/'

    path_out_ims = osp.join(path_data_out, 'test_images')
    path_out_fovs = osp.join(path_data_out, 'test_fovs')
    ...
    df_od.drop(['center', 'DR_Level'], axis=1).to_csv(osp.join(path_data_out, 'test_od.csv'), index=False)
    df_mac.drop(['center', 'DR_Level'], axis=1).to_csv(osp.join(path_data_out, 'test_mac.csv'), index=False)
