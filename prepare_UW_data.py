...
if __name__ == "__main__":
    ...
    path_out_ims = osp.join(path_data_out, 'images_uw')
    path_out_fovs = osp.join(path_data_out, 'fovs_uw')
    os.makedirs(path_out_ims, exist_ok=True)
    os.makedirs(path_out_fovs, exist_ok=True)

    print('Preparing training data')
    csv_path = osp.join(path_raw_data, 'ultra-widefield-training/ultra-widefield-training.csv')

    df = pd.read_csv(csv_path)
    df = df.drop(['position'], axis=1)
    df['dr'] = df['DR_level']
    df['center'] = ['od' if '1.jpg' in n else 'mac' for n in list(df.image_path)]
    abs_path = [osp.join(path_raw_data, n.replace('\\', '/')) for n in list(df.image_path)]
    ...
    df_od = df[df.center == 'od']
    df_mac = df[df.center == 'mac']
    df_od.drop(['center'], axis=1).to_csv(osp.join(path_data_out,'train_od_UW.csv'), index=False)
    df_mac.drop(['center'], axis=1).to_csv(osp.join(path_data_out, 'train_mac_UW.csv'), index=False)
    print('Training data prepared')

    print('Preparing validation data')
    csv_path = osp.join(path_raw_data, 'ultra-widefield-validation/ultra-widefield-validation.csv')
    ...
    df_od = df[df.center == 'od']
    df_mac = df[df.center == 'mac']
    df_od.drop(['center'], axis=1).to_csv(osp.join(path_data_out, 'val_od_UW.csv'), index=False)
    df_mac.drop(['center'], axis=1).to_csv(osp.join(path_data_out, 'val_mac_UW.csv'), index=False)
