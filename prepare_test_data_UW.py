...
if __name__ == "__main__":
    ...
    path_out_ims = osp.join(path_data_out, 'test_images_UW')
    path_out_fovs = osp.join(path_data_out, 'test_fovs_UW')
    ...
    df_od['image_id'] = df_od['image_id'].apply(lambda x: 'data/test_images_UW/' + x + '.jpg')
    df_mac['image_id'] = df_mac['image_id'].apply(lambda x: 'data/test_images_UW/' + x + '.jpg')

    df_od.drop(['center', 'DR_level'], axis=1).to_csv(osp.join(path_data_out, 'test_od_UW.csv'), index=False)
    df_mac.drop(['center', 'DR_level'], axis=1).to_csv(osp.join(path_data_out, 'test_mac_UW.csv'), index=False)
